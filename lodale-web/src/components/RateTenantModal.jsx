...
export default function RateTenantModal({
  isOpen,
  onClose,
  tenantId,
  tenantName = "Tenant",
  ...
  const handleSubmit = (e) => {
    ...
    try {
      const res = ratingService.submitTenantReview({
        tenantId,
        tenantName,
        landlordId,
        landlordName,
      ...
      if (res.success) {
        triggerToast("Thank you! Your tenant rating has been recorded successfully.", "success", "Rating Saved");
      ...
      console.error(err);
      triggerToast("An error occurred while submitting tenant rating.", "error");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between pb-3 border-b border-ink-100 dark:border-white/10">
          <div>
            <h3 className="text-lg font-bold text-ink-900 dark:text-white flex items-center gap-2">
              <Star className="h-5 w-5 fill-amber-400 text-amber-400" /> Rate & Review Tenant
            </h3>
            <p className="text-xs text-ink-500 dark:text-cream-100/60 mt-0.5">
              {tenantName} {propertyTitle ? `• ${propertyTitle}` : ''}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-center space-y-2">
            ...
            <span className="text-xs font-semibold text-moss-700 dark:text-[#E5C583]">
              {rating === 5 ? '5.0 - Excellent & Punctual Tenant' : rating === 4 ? '4.0 - Good Tenant' : rating === 3 ? '3.0 - Satisfactory' : rating === 2 ? '2.0 - Below Expectations' : '1.0 - Poor'}
            </span>
          </div>
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              placeholder="Share details about payment punctuality, property care, rule compliance, and communication..."
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-2.5 px-3 rounded-xl border text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${wouldRentAgain
