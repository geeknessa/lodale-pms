...
export default function LandlordDashboard() {
  ...
  const currentDateStr = formatDateShort(new Date());
  ...
  }, [runTour, tourStep, recalcTrigger, activeTab, updateTourPosition]);
                ...
                <div className="db-popup-body">
                  <div className="db-popup-stats-summary">
                    <div style={{ flex: 1 }}>
                      ...
                      <span className="db-popup-stats-val">{formatCurrency(collectedAmount)}</span>
                    </div>
                    <div style={{ flex: 1, borderLeft: "1.5px solid var(--border-light)", paddingLeft: "16px" }}>
                      ...
                      <span className="db-popup-stats-val overdue">{formatCurrency(outstandingAmount)}</span>
                    </div>
                  </div>
                            ...
                            <div key={t.id || idx} className="db-popup-item">
                              <div className="db-popup-info">
                                <div className="db-popup-name-row">
                                  ...
                                  <span className="db-popup-status-text paid">+{formatCurrency(amount)}</span>
                                </div>
                          ...
                          return (
                            <div key={t.id || idx} className="db-popup-item">
                              <div className="db-popup-info">
                                <div className="db-popup-name-row">
                                  <span className="db-popup-name">{t.name || t.tenantName}</span>
                                  <span className="db-popup-status-text overdue">{formatCurrency(amount)}</span>
                                </div>
                                <p className="db-popup-property">{t.propertyTitle || t.leaseStatus || "Leased Property"}</p>
            ...
            return (
              ...
              >
                {displayTenants.length > 0 ? (
                  displayTenants.map((t, idx) =>
                    t.avatar ? (
                      <img
                        ...
                        src={t.avatar}
                        alt={t.name || "Tenant"}
                        className="w-7 h-7 rounded-full object-cover border-2 border-white dark:border-[#263b33]"
                        onError={(e) => {
                      ...
                      <div
                        key={t.id || idx}
                        className={`w-7 h-7 rounded-full ${idx % 2 === 0 ? "bg-moss-700" : "bg-amber-600"} text-white flex items-center justify-center text-xs font-bold border-2 border-white dark:border-[#263b33]`}
                      >
                  ...
                  <>
                    <div className="w-7 h-7 rounded-full bg-moss-700 text-white flex items-center justify-center text-xs font-bold border-2 border-white dark:border-[#263b33]">
                      <User className="h-3.5 w-3.5" />
                    </div>
                    <div className="w-7 h-7 rounded-full bg-amber-600 text-white flex items-center justify-center text-xs font-bold border-2 border-white dark:border-[#263b33]">
                      <User className="h-3.5 w-3.5" />
                    </div>
                  </>
                )}
                ...
                <div className="flex items-center justify-between pb-3 border-b border-[#E4EAE1] dark:border-white/10">
                  <h3 className="font-bold text-sm text-ink-900 dark:text-white flex items-center gap-2">
                    ...
                    {notifications.length > 0 && (
                      <span className="px-2 py-0.5 text-[10px] font-extrabold bg-[#2C4633] text-white dark:bg-[#E5C583] dark:text-[#263b33] rounded-full">
                  ...
                  <section className="db-card activity-card tour-occupancy">
                    ...
                    <div className="activity-chart-grid">
                      {[
                        ...
                        { day: "Fri", base: 95, highlight: true },
                ...
                <div className="db-col">
                  <div className="visa-card-mock tour-vault">
                    ...
                    <div className="visa-card-body">
                      ...
                      <p className="visa-card-amount">{formatCurrency(availablePayoutBalance)}</p>
                    </div>
      ...
      {showRatingModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-md p-4">
          <div className="bg-white dark:bg-[#12221C] rounded-3xl border border-[#E4EAE1] dark:border-white/10 max-w-md w-full p-8 shadow-2xl relative text-center">
            ...
            <button
              onClick={() => setShowRatingModal(false)}
              className="mt-6 w-full py-3 bg-[#2C4633] dark:bg-[#E5C583] hover:bg-[#1C3021] dark:hover:bg-[#d4b574] text-white dark:text-[#263b33] font-bold text-[13.5px] rounded-xl cursor-pointer transition-all duration-150 active:scale-[0.98] border-none outline-none"
            >
              Close Reviews
            </button>
          </div>
      ...
      {showLandlordProfileModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-md p-4">
          <div className="bg-white dark:bg-[#12221C] rounded-3xl border border-[#E4EAE1] dark:border-white/10 max-w-sm w-full p-8 shadow-2xl relative text-center">
            ...
            <button
              onClick={() => setShowLandlordProfileModal(false)}
              className="mt-6 w-full py-3 bg-[#2C4633] dark:bg-[#E5C583] hover:bg-[#1C3021] dark:hover:bg-[#d4b574] text-white dark:text-[#263b33] font-bold text-[13.5px] rounded-xl cursor-pointer transition-all duration-150 active:scale-[0.98] shadow-md shadow-[#2C4633]/10 border-none outline-none"
            >
              Close Profile
            </button>
          </div>
